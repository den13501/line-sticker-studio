...
const CHROMA_KEYS = {
  green: {
    name: "green screen",
    forbiddenLabel: "green-screen color",
    hex: "#00FF00",
    color: "PURE NEON GREEN",
  ...
  magenta: {
    name: "magenta screen",
    forbiddenLabel: "magenta-screen color",
    hex: "#FF00FF",
    color: "PURE NEON MAGENTA",
...
- Background is plain solid ${key.color} (${key.hex}) — this is a ${key.name} chroma-key plate that will be programmatically removed by the downstream tool. Use exactly ${key.hex} only for the background plate. NO gradients, NO shading, NO scenery, NO patterns, NO ground plane, NO platform, NO cast shadow, NO contact shadow. Same identical ${key.hex} color across all 9 cells.
- CRITICAL: the character itself must contain NO ${key.forbiddenLabel} anywhere. Avoid ${key.avoid} on clothes, hair, eyes, accessories, props, highlights, reflected light, and shadows. If the original reference has colors too close to ${key.hex}, substitute them with ${key.substitutions}. Even slight key-color tints on white clothes or skin should be avoided. This is essential — pixels matching ${key.hex} will be chroma-keyed out and become holes.
- Do not use ${key.hex} or ${key.avoid} for the character, outlines, highlights, shadows, reflected light, props, floor, ground, platforms, leaves, grass, objects, or any area inside the character silhouette. Only the removable background plate may use ${key.hex}.
